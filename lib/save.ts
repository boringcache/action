...
async function run(): Promise<void> {
  try {
    ...
    if (cacheEntries && workspace) {
      ...
      await saveCache(workspace, cacheEntries, false, false, false, false);
    } else {
      ...
      await saveCache(workspace, entriesString, inputs.force, inputs.noPlatform, inputs.verbose, inputs.enableCrossOsArchive);
    }
...
async function saveCache(workspace: string, entries: string, force: boolean = false, noPlatform: boolean = false, verbose: boolean = false, enableCrossOsArchive: boolean = false): Promise<void> {
